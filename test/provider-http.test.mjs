...
test("safe GET retries transient responses and Retry-After is never jittered early", async () => {
  const delays = [];
  let calls = 0;
  const request = createHttpClient({
    fetch: async () => ++calls === 1
      ? jsonResponse(429, { error: { message: "wait" } }, { "retry-after": "0.08" })
  ...
  assert.equal(retryDelayMs({ retryAfter: "2", now, attempt: 1, maxDelayMs: 5000, jitterRatio: 0.5, random: () => 0 }), 2000);
  assert.equal(retryDelayMs({ retryAfter: "Wed, 01 Jan 2025 00:00:02 GMT", now, attempt: 1, maxDelayMs: 5000, jitterRatio: 0.5, random: () => 1 }), 3000);
  assert.equal(retryDelayMs({ retryAfter: "Wed, 01 Jan 2025 00:00:20 GMT", now, attempt: 1, maxDelayMs: 1000, jitterRatio: 0.2, random: () => 0 }), 1000);
});

test("ambiguous paid POST network, body, and 5xx failures are never retried", async (t) => {
  for (const scenario of ["network", "body", "5xx", "generic-capacity"]) {
    await t.test(scenario, async () => {
      ...
      const request = createHttpClient({ fetch: async () => {
        ...
        return jsonResponse(503, { error: { message: scenario === "generic-capacity" ? "capacity temporarily unavailable" : "internal failure" } });
